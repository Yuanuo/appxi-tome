...
public class ChmFile implements AutoCloseable {
    ...
    private void readInitialHeaderAndDirectory() throws IOException {
        ...
        Log.log(String.format("Language ID: 0x%x", itsfHeader.langId));
    ...
    private void readResetTable() throws IOException {
        ...
        if (uiResetTable == null || uiResetTable.space == CHM_COMPRESSED ||
            ...
            Log.log("Compression is disabled.");
        ...
        if (buffer == null || buffer.remaining() < CHM_LZXC_RESETTABLE_V1_LEN) {
            Log.log("Failed to retrieve reset table.");
            compressionDisabled = true;
            return;
        }
    ...
    private void readControlData() throws IOException {
        ChmUnitInfo uiLzxc = resolveObject(CHMU_LZXC_CONTROLDATA);
        if (uiLzxc == null) {
            Log.log("No LZXC control data found.");
            compressionDisabled = true;
            return;
        }
        ...
        if (buffer == null) {
            Log.log("Failed to retrieve LZXC control data");
            compressionDisabled = true;
            return;
        }
    ...
    private void initMiscFiles(String filename) {
        ...
        if (encoding == null || encoding.length() == 0) {
            ...
            Log.log("Fallback Encoding: " + encoding);
            ...
            if (!encodingInItsfHeader.startsWith("CP")) { // better
                ...
                Log.log("Fixed Encoding: " + encoding);
            }
        }
        if (generator == null) {
            generator = "";
    ...
    private void initMiscFilesWithoutCatch() throws IOException {
        ...
        if (system == null) {
            Log.log("The #SYSTEM object doesn't exist.");
            return;
        }
        ...
        while (buf.hasRemaining()) {
            ...
            switch (type) {
                ...
                case 4:
                    ...
                    Log.log(String.format("Language ID: 0x%x", detectedLCID));
                    Log.log("Encoding: " + encoding);
                    ByteBufferHelper.skip(buf, len - 4);
                    break;
                case 9:
                    generator = ByteBufferHelper.parseString(buf, len, encoding);
                    Log.log("Generator: " + generator);
                    break;
                default:
                    // Log.log(String.format("Unknown type: %d, len %d", type, len));
    ...
    private synchronized ByteBuffer fetchBytes(long offset, long len) {
        try {
            ...
            Log.log("Failed to fetch bytes: " + e);
            return null;
        }
    }
